<%@ Page Language="VB" AutoEventWireup="false" CodeFile="PendingInterestReport.aspx.vb" Inherits="mmsoft_PendingInterestReport" %>
                ...
                <table width="100%">
                    <tr>
                        <td colspan="3" style="height: 21px; text-align: center">
                            <span style="font-size: 8pt; font-family: Verdana"><strong>Pending Interest Details</strong></span></td>
                    </tr>
                    <tr>
                        <td colspan="3">
                            <table width="100%">
                                ...
                                <tr>
                                    <td colspan="5">
                                        <asp:GridView ID="Grid2" runat="server" AllowPaging="True" CellPadding="4" DataSourceID="SqlDataSource1"
                                            ...
                                            <Columns>
                                                ...
                                                <asp:BoundField DataField="TotalPayment" HeaderText="Scheduled Amt" SortExpression="TotalPayment" />
                                                ...
                                                <asp:BoundField DataField="PymtMode" HeaderText="Mode" SortExpression="PymtMode" />
                                                <asp:BoundField DataField="PymtAmount" HeaderText="Paid Amt" SortExpression="PymtAmount" />
                                                <asp:BoundField DataField="IntPending" HeaderText="Pending Int" SortExpression="IntPending" />
                                                <asp:BoundField DataField="intDays" HeaderText="Int Days" SortExpression="intDays" />
                                        ...
                                        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:conMM %>"
                                            SelectCommand="SELECT PymtEntry.PymtEntryId, PymtEntry.PymtId, Pyment.RoomId, salesDetails.FName, salesDetails.LName, Building.BldgName, stage.StageDesc, PymtEntry.PymtDate, PymtEntry.PymtMode, PymtEntry.PymtAmount, PymtEntry.IntPending, PymtEntry.intDays, Pyment.DueDate, Pyment.TotalPayment, Pyment.OutStanding FROM BldgStage INNER JOIN stage ON BldgStage.StageId = stage.StageId INNER JOIN Pyment INNER JOIN PymtEntry ON Pyment.PymtId = PymtEntry.PymtId INNER JOIN salesDetails ON Pyment.RoomId = salesDetails.RoomId ON BldgStage.BldgStageId = Pyment.BldgStageId INNER JOIN Building ON salesDetails.BldgId = Building.BldgId WHERE (Pyment.RoomId = @RoomId) ORDER BY Pyment.RoomId">
                                            <SelectParameters>
