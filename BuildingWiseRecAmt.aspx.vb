...
Partial Class mmsoft_BuildingWiseRecAmt
    ...
    Protected Sub imbSave_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbSave.Click
        ...
        If Not df Is Nothing Then
            For i = 0 To dx.Rows.Count - 1
                ...
                If status Then
                    str = "SELECT TOP 100 PERCENT SUM(dbo.PymtEntry.PymtAmount) AS Expr1, dbo.Building.BldgId FROM dbo.PymtEntry INNER JOIN"
                    str = str + " dbo.Pyment ON dbo.PymtEntry.PymtId = dbo.Pyment.PymtId INNER JOIN "
                    str = str + " dbo.BldgStage ON dbo.Pyment.BldgStageId = dbo.BldgStage.BldgStageId INNER JOIN"
                    str = str + " dbo.Building ON dbo.BldgStage.BldgId = dbo.Building.BldgId"
                    str = str + " GROUP BY dbo.Building.BldgId, dbo.PymtEntry.PymtDate "
                    str = str + " HAVING (dbo.PymtEntry.PymtDate BETWEEN '" & fromdate.Date & "' AND '" & todate.Date & "') AND "
                    str = str + "  (dbo.Building.BldgId = " & dx.Rows(i)("BldgId").ToString() & ") ORDER BY dbo.Building.BldgId"
                    ...
                    Dim aggamount As Integer
                    aggamount = 0
                    Dim x As Integer
                    Dim dt As DataTable
                    ds = objMid.ExecuteSelectSQL(str)
                    dt = ds.Tables(0)
                    If Not ds Is Nothing Then
                        For x = 0 To dt.Rows.Count - 1


                            aggamount = aggamount + dt.Rows(x)("Expr1").ToString()

                            'lblbuilding.Text = lblbuilding.Text + Str(frommonth) + " : year: " + Str(fromyear) + " : " + dt.Rows(0)(0).ToString()

                        Next
                    End If
                    myrow("Building / Row House") = dx.Rows(i)("BldgName").ToString()
                    'myrow("Month") = getMonth(frommonth)
                    myrow("Amount") = aggamount
                    'If frommonth = 12 Then
                    '    fromyear = fromyear + 1
                    '    frommonth = 1
                    'Else
    ...
    Public Function GetAdvance(ByVal frommonth As Integer, ByVal fromyear As Integer) As Integer
        Dim strsql As String
        Dim gnds As New DataSet
        'Dim gndt As New DataTable
        strsql = "select pymtamount from pymtAdventry where PymtDate between '" & Str(frommonth) & "/1/" & Str(fromyear) & "' and '" & Str(1) & "/1/" & Str(fromyear + 1) & "'"
    ...
    Public Sub CreateDefaultersTbl()
        ...
        FlatNos = New DataColumn("Building / Row House", System.Type.GetType("System.String"))
