...
Partial Class mmsoft_CreateBill
    ...
    Protected Sub drpFlatNumber_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles drpFlatNumber.SelectedIndexChanged
        ...
        'End If
        lblReceiptNo.Text = getReceiptNo("BillDetails", "ReceiptNos")
    ...
    Public Function CheckValid(ByVal RecpNo As String) As Boolean
        ...
        dsparti = objMid.ExecuteSelectSQL("select ReceiptNos from BillDetails where ReceiptNos='" & RecpNo & "'")
    ...
    Public Function getReceiptNo(ByVal TableName As String, ByVal ColumnName As String) As Integer
        ...
        myID = IIf(myDataSet.Tables(0).Rows(0).Item(0).ToString = "", 3164, myDataSet.Tables(0).Rows(0).Item(0))
        Return myID
    End Function
    Public Sub CheckBillDuplicate(ByVal Billno)
        Dim conn As New SqlConnection(constr)
        Dim dt As New DataTable

        Try
            conn.Open()
            Dim sql As String = "SELECT * FROM BillDetails WHERE ReceiptNos =@ReceiptNos"
            Dim cmd As New SqlCommand(sql, conn)
            cmd.Parameters.AddWithValue("@ReceiptNos", Billno)
    ...
    Protected Sub imbSave_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbSave.Click
        ...
        If CheckValid(lblReceiptNo.Text) = False Then
            ...
            Exit Sub
        End If
        CheckBillDuplicate(lblReceiptNo.Text)
        If myflag = True Then


            Dim sql As String
            sql = "Insert BillDetails (ReceiptNos,ReceiptDate,RoomId,ReceiptAmt,AmtInLetters,ModeOfPayment,BankName,BranchName,OtherDetails,chequedate,UserId,CreatedDate) Values("
            sql = sql & lblReceiptNo.Text & ",'" & ReceiptDate.Date & "'," & drpFlatNumber.SelectedValue & "," & txtamount.Text & ",'" & txtAmtInWords.Text & "',"
            sql = sql & "'" & drpMode.SelectedItem.Text & "','" & txtBank.Text & "','" & txtBranch.Text & "','" & txtotherdetails.Text & "','" & cheqdate.Date & "' ,'" & Session("USERID") & "','" & Now.Date & "')"
            ...
            For Each gvRow As DataRow In ParticularsDetails.Rows
                ...
                objMid.ExecuteSqlStmt("Insert Into ParticularsTable(particulartblId,ReceiptNos,ParticularsId,ParticularsAmt) values(" & objMid.getUniqueID("ParticularsTable", "particulartblId") & "," & lblReceiptNo.Text & ",'" & GetParticularsId(particulars) & "'," & amount & ")")

            Next
            Session("ReceiptNos") = lblReceiptNo.Text
            Response.Redirect("CreateBillLayout.aspx")
        Else
            lblmsg.Text = "Dupicate Bill"
        End If
